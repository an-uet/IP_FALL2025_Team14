...
class NaiveBackgroundStyleTransfer():
    ...
    """
    def __init__(self, number_of_epochs=1000, content_weight=1e3, style_weight=1e-1, model_name="mobilenetv2_coco_voctrainaug", enable_gpu=False, verbose=False, name=None):
    ...
    def generate_gif(self, speed, name):
        """
        Generates a gif image.
        """
        return self._generate_gif(speed, name)
    ...
    def _perform(self, content_file, style_file):
        ...
        with tf.device(self.device_name):
            ...
            for epoch in range(self.number_of_epochs):
              ...
              plt.close()
            imageio.imwrite(self.path.output_image_path + self.name + '.jpg', final_image)
            return final_image
    ...
    def _generate_gif(self, speed, name):
        """
        Generate gif Image.
        """
        ...
        # latest_image_path = filenames[-1]  # file cuối
        # img = imageio.imread(latest_image_path)

        # result_path = os.path.join(self.path.output_gif_path, f"{name}.jpg")
